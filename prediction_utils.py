...
def get_weights_filename(path_model, loss_name, verbose=False, num_file=None):
    weights = sorted([w for w in os.listdir(
        path_model + '/weights') if 'index' in w])
    if verbose:
        print(weights)
    if loss_name is not None:
        ...
        # weights = weights[0]
            ...
            if verbose:
                print('weights by monitor')
            weights = max(weights, key=lambda w: [float(s[3:]) for s in w.replace(
                '.ckpt.index', '').split('-') if s.startswith('mon')][0])
        elif 'val_loss' not in weights[0]:
            if verbose:
                print('weights by last')
            weights = weights[-1]
        else:
            if verbose:
                print('weights by val_loss')
...
def load_model(path_model, return_sequences=True, num_file=None, loss_name=None):
    ...
    scale_data = model_params.get(
        'scale_data', False) or model_params.get('use_scaler', False)
    if scale_data:
        print(' * Loading data scaler')
        model_params['scaler'] = pickle.load(
            open(path_model + 'scaler.pckl', 'rb'))
    else:
    ...
    for data_key in ['use_jcd_features', 'use_speeds', 'use_coords_raw',
        ...
        if data_key not in model_params:
            model_params[data_key] = False

    if 'average_wrong_skels' not in model_params:
