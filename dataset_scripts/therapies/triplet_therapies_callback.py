...
from eval_scripts import eval_utils
...
def get_therapies_triplet_distances(model, model_params, triplets, triplets_bgnd, video_skels,
    ...
    for vid, (tempos, skels) in video_skels.items():
        ...
        if not in_memory_callback or vid not in cached_videos:
            ...
            if in_memory_callback:
                cached_videos[vid] = skels
        else:
            skels = cached_videos[vid]
    ...
    for A, P, N in triplets:
        triplet_embs.append(
            (video_preds[A[0]][A[2]], video_preds[P[0]][P[2]], video_preds[N[0]][N[2]]))

    triplet_bgnd_embs = []
    for A, P, N in triplets_bgnd:
        triplet_bgnd_embs.append(
            (video_preds[A[0]][A[2]], video_preds[P[0]][P[2]], video_preds[N[0]][N[2]]))
...
def get_therapies_triplet_distances_by_samples(model, model_params, triplets, triplets_bgnd, video_skels,
    ...
    triplets_flat = ['|'.join(map(str, tup))
                     for t in triplets + triplets_bgnd for tup in t]
    ...
    triplet_embs = []
    for A, P, N in triplets:
        triplet_embs.append((samples_embs['|'.join(map(str, A))], samples_embs['|'.join(
            map(str, P))], samples_embs['|'.join(map(str, N))]))

    triplet_bgnd_embs = []
    for A, P, N in triplets_bgnd:
        triplet_bgnd_embs.append((samples_embs['|'.join(map(str, A))], samples_embs['|'.join(
            map(str, P))], samples_embs['|'.join(map(str, N))]))
...
def eval_therapies_triplet_callback(model, model_params, writer, mode):
    ...
    triplets = pickle.load(
        open(model_params['eval_therapies_triplets_dataset'], 'rb'))
    triplets_bgnd = pickle.load(
        open(model_params['eval_therapies_triplets_bgnd_dataset'], 'rb'))
    video_skels = pickle.load(
        open(model_params['eval_therapies_video_skels'], 'rb'))

    in_memory_callback = model_params['in_memory_callback']
    if in_memory_callback:
        print(' ** OneShot-Therapies Callback | {} | data will be cached **'.format(mode))
    ...
    if mode == 'full':
        ...
        raise ValueError('Callback mode not recognized')

    scale_data = model_params['scale_data']
    if scale_data:
        model_params['scaler'] = load_scaler(**model_params)
    else:
        model_params['scaler'] = None
    ...
    def eval_therapies(epoch, logs):
        ...
        total_stats.append({'ther_trip_{}_euc_{}'.format(
            mode, k): v for k, v in stats.items()})
        stats = eval_utils.eval_triplet_embeddings(triplet_embs, 'cosine')
        total_stats.append({'ther_trip_{}_cos_{}'.format(
            mode, k): v for k, v in stats.items()})
        stats = eval_utils.eval_triplet_embeddings(
            triplet_embs, 'jensenshannon')
        total_stats.append({'ther_trip_{}_js_{}'.format(
            mode, k): v for k, v in stats.items()})
        txt = ' ** Triplet therapies {} ** '.format(mode)
        txt += ' ||| Euc.: {:.2f}% | diff. {:.2f} ||| Cos.: {:.2f}% | diff. {:.2f} ||| JS.: {:.2f}% | diff. {:.2f} ||| '.format(
            total_stats[0]['ther_trip_{}_euc_{}'.format(
                mode, 'acc')], total_stats[0]['ther_trip_{}_euc_{}'.format(mode, 'dist_diff')],
            total_stats[1]['ther_trip_{}_cos_{}'.format(
                mode, 'acc')], total_stats[1]['ther_trip_{}_cos_{}'.format(mode, 'dist_diff')],
        ...
        stats = eval_utils.eval_triplet_embeddings(
            triplet_bgnd_embs, 'euclidean')
        total_stats.append({'ther_trip_bgnd_{}_euc_{}'.format(
            mode, k): v for k, v in stats.items()})
        stats = eval_utils.eval_triplet_embeddings(triplet_bgnd_embs, 'cosine')
        total_stats.append({'ther_trip_bgnd_{}_cos_{}'.format(
            mode, k): v for k, v in stats.items()})
        stats = eval_utils.eval_triplet_embeddings(
            triplet_bgnd_embs, 'jensenshannon')
        total_stats.append({'ther_trip_bgnd_{}_js_{}'.format(
            mode, k): v for k, v in stats.items()})
        txt += ' ** BGND ** Euc.: {:.2f}% | diff. {:.2f} ||| Cos.: {:.2f}% | diff. {:.2f} ||| JS.: {:.2f}% | diff. {:.2f}'.format(
            total_stats[3]['ther_trip_bgnd_{}_euc_{}'.format(
                mode, 'acc')], total_stats[3]['ther_trip_bgnd_{}_euc_{}'.format(mode, 'dist_diff')],
            total_stats[4]['ther_trip_bgnd_{}_cos_{}'.format(
                mode, 'acc')], total_stats[4]['ther_trip_bgnd_{}_cos_{}'.format(mode, 'dist_diff')],
