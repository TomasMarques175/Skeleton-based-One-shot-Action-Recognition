...
if __name__ == "__main__":
    ...
    static_params = {
        ...
        "model_name": "Models_MP_APPDA_Block_0",
    ...
    # === Inspect a single batch to verify feature dimensions ===
    first_batch = next(iter(full_loader))
    batch_x, batch_y = first_batch

    print(f"Batch X shape: {batch_x.shape}")  # (batch_size, num_feats, seq_len)
    print(f"Batch Y shape: {batch_y.shape}")

    # Optional: also check one sample directly from the dataset
    sample_x, sample_y = full_dataset[0]
    print(f"Single sample X shape: {sample_x.shape}, label: {sample_y}")
