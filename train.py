...
def main(model_params):
    ...
    log_interval = model_params.get('log_interval', 10) # Log every N batches

    # --- Path and Feature Calculation ---
    model_params['path_model'] = train_utils.create_model_folder(
        model_params['path_results'], model_params['model_name']
    )
    # Recalculate num_feats based on current params to ensure correctness
    try:
        # This is crucial: get_num_feats must accurately reflect the features
        # generated by get_pose_data_processed in your Dataset.
        ...
        if model_params.get('num_feats') != calculated_num_feats and model_params.get('num_feats') is not None :
             print(f"Warning: model_params['num_feats'] ({model_params.get('num_feats')}) "
                   f"differs from get_num_feats calculation ({calculated_num_feats}). "
                   f"Using calculated value: {calculated_num_feats}.")
        ...
        # Fallback to a pre-set num_feats if calculation fails, but this is risky
        if 'num_feats' not in model_params or model_params['num_feats'] is None:
            print("CRITICAL: num_feats not set and get_num_feats failed. Aborting.")
            return
        os.makedirs(model_params['path_model'], exist_ok=True)
    json.dump(model_params, open(os.path.join(model_params['path_model'], 'model_params.json'), 'w'))
    print(' * Model params used for this run:', json.dumps(model_params, indent=2))

    try:
        with open(model_params['train_annotations'], 'r') as f: num_train_files = len(f.read().splitlines())
        if not model_params.get('val_annotations'): num_val_files = 0
        else:
        ...
        print(f"Error: Annotation file not found: {e}. Please check paths in model_params.")
        return

    if model_params.get('scale_data', False):
        try:
            scaler_filename = get_scaler_filename(**model_params)
            scaler_save_path = os.path.join(model_params['path_model'], 'scaler.pckl')
            if os.path.exists(scaler_filename):
                copyfile(scaler_filename, scaler_save_path)
                print(f"Copied scaler file: {scaler_filename} to {scaler_save_path}")
            else:
                print(f"Warning: Scaler source file {scaler_filename} not found. Not copied.")
        except Exception as e:
            print(f"Warning: Error handling scaler file: {e}. Skipping scaler copy.")


    # --- Double check This ---
    model_params.update({'num_jcd_feats': int(comb(model_params['joints_num'],2))})
    ...
    print("\n--- Creating PyTorch TCN_clf model ---")
    model = TCN_clf(
    ...
    # --- Model Sanity Check & Summary ---
    _test_batch_size = model_params.get('batch_size_test', 2)
    _test_seq_len = abs(model_params.get('max_seq_len', 32))
    if _test_seq_len == 0: _test_seq_len = 32
    _test_num_feats = model_params['num_feats']
    summary_input_shape = (_test_batch_size, _test_seq_len, _test_num_feats)
    try:
        print(f"\nModel Summary (input shape for summary: {summary_input_shape}):")
        summary(model, input_size=summary_input_shape, col_names=["input_size", "output_size", "num_params", "kernel_size"], verbose=0) # Removed mult_adds for brevity
    except Exception as e:
        print(f"Could not print model summary using torchinfo: {e}.\nBasic model structure:\n{model}")

    # --- PyTorch Setup for Training ---
    ...
    if model_params.get('triplet', False):
        # Note: Triplet loss requires batch mining logic in the training loop
    ...
    loss_weights_pytorch = {
        'classification': model_params.get('clf_loss_weight', 1.0 if 'classification' in active_losses and not 'triplet' in active_losses else 0.4),
        'triplet': model_params.get('triplet_loss_weight', 1.0 if 'triplet' in active_losses and not 'classification' in active_losses else 0.6)
    }
    print(f"Losses: {list(active_losses.keys())}, Weights: {loss_weights_pytorch}")
