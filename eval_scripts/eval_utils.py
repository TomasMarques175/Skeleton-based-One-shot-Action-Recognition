...
=======
    for A, P, N in triplet_embeddings:
        res.append((get_distance(dist_func, A, P),
                   get_distance(dist_func, A, N)))
>>>>>>> 4cefc2f (- requirements.txt file with all the dependencies in order to create an python env that can easily run the code)
...
=======
    ...
    if show_joint_txt:
        for i in range(len(skel)):
            ax.text(skel[i][0], skel[i][1], skel[i][2], str(i))
...
def print_3d_skeleton(ax, skel, color=None, color_scatter=None,
    ...
    if color_scatter is not None:
        color_scatter = [color_scatter] * len(skel)
    ax.scatter(skel[:, 0], skel[:, 1], skel[:, 2], alpha=scatter_alpha,
               s=scatter_size, c=color_scatter)        # , c=color
