...
=======
        ...
        if k_acc not in ea.tensors.Keys():
            continue

        step = max(ea.Tensors(k_acc),
                   key=lambda x: tf.make_ndarray(x.tensor_proto)).step
        acc = [float(tf.make_ndarray(t.tensor_proto))
               for t in ea.Tensors(k_acc) if t.step == step][0]
        dist_p = [float(tf.make_ndarray(t.tensor_proto))
                  for t in ea.Tensors(k_p) if t.step == step][0]
        dist_n = [float(tf.make_ndarray(t.tensor_proto))
                  for t in ea.Tensors(k_n) if t.step == step][0]

        thr_med = (dist_p + dist_n) / 2
        thr_excel = dist_p
        thr_good = (thr_med + thr_excel) / 2
        metric_thr[metric] = {'med': thr_med,
                              'good': thr_good, 'excel': thr_excel}
    return metric_thr


def get_video_distances(pf, actions_data, video_skels, model, model_params, batch=None, in_memory_callback=False, cache={}):

    anchors = actions_data[(actions_data.preds_file == pf)
                           & (actions_data.is_therapist == 'y')]
    anchors_preds_filename = anchors.preds_filename.drop_duplicates().tolist()[0]

    if not in_memory_callback or not anchors_preds_filename in cache:
        anchors_tempos, anchors_num_frames, anchors_skels_raw = video_skels[
            anchors_preds_filename]
        if model_params['average_wrong_skels']:
            anchors_skels_raw = average_wrong_frame_skels(anchors_skels_raw)
        anchors_skels = get_pose_data_v2(
            anchors_skels_raw, validation=True, **model_params)
        # anchors_skels = np.expand_dims(anchors_skels, axis=0)
        # anchor_preds = np.array(model.get_embedding(anchors_skels))[0]
        anchors_y_true = [any((anchors.init_frame < num_frame) & (anchors.end_frame > num_frame))
                          for num_frame in anchors_num_frames]
...
=======
        for _, row in anchors.iterrows():
            data_anchor.loc[(data_anchor.num_frame > row.init_frame) & (
                data_anchor.num_frame < row.end_frame), 'action_id'] = row.comments
        if in_memory_callback:
            cache[anchors_preds_filename] = data_anchor
    else:
        data_anchor = cache[anchors_preds_filename]

    targets = actions_data[(actions_data.preds_file == pf)
                           & (actions_data.is_therapist == 'n')]
    targets_preds_filename = anchors_preds_filename.replace(
        'keypoints', 'keypointschild')
    if not targets_preds_filename in cache:
        targets_tempos, targets_num_frames, targets_skels_raw = video_skels[
            targets_preds_filename]
        if model_params['average_wrong_skels']:
            targets_skels_raw = average_wrong_frame_skels(targets_skels_raw)
        targets_skels = get_pose_data_v2(
            targets_skels_raw, validation=True, **model_params)
        # targets_skels = np.expand_dims(targets_skels, axis=0)
        # targets_preds = np.array(model.get_embedding(targets_skels))[0]
        targets_is_target = [any((targets.init_frame < num_frame) & (targets.end_frame > num_frame))
                             for num_frame in targets_num_frames]
...
=======
        ...
        for _, row in targets.iterrows():
            data_target.loc[(data_target.num_frame > row.init_frame) & (
                data_target.num_frame < row.end_frame), 'target_id'] = row.comments
        for _, row in anchors.iterrows():
            data_target.loc[(data_target.num_frame > row.init_frame) & (
                data_target.num_frame < row.end_frame), 'anchor_id'] = row.comments
>>>>>>> 4cefc2f (- requirements.txt file with all the dependencies in order to create an python env that can easily run the code)
...
=======
    ...
    data_anchor.loc[:, 'emb'] = pd.Series(list(np.array(model.get_embedding(
        np.expand_dims(np.array(data_anchor.skels_feats.tolist()), axis=0), batch=batch))[0]))
    data_target.loc[:, 'emb'] = pd.Series(list(np.array(model.get_embedding(
        np.expand_dims(np.array(data_target.skels_feats.tolist()), axis=0), batch=batch))[0]))
>>>>>>> 4cefc2f (- requirements.txt file with all the dependencies in order to create an python env that can easily run the code)
...
def get_anchor_embs_by_strategy(data_anchor, anchors_info, anchor_strategy):
    if anchor_strategy == 'last':  # Last anchor frame
        anchor_embs = {row.end_frame: [data_anchor[data_anchor.action_id ==
                                                   row.comments].emb.iloc[-1]] for _, row in anchors_info.iterrows()}
    # Pick embeddings by position percentage
    elif anchor_strategy.startswith('perc'):
        percs = list(map(float, anchor_strategy[5:].split('_')))
        anchor_embs = {}
        for _, row in anchors_info.iterrows():
            frame_embs = data_anchor[data_anchor.action_id == row.comments].emb
            anchor_embs[row.end_frame] = [
                frame_embs.iloc[int((len(frame_embs)-1)*perc)] for perc in percs]
    # Pick embeddings by position percentage
    elif anchor_strategy.startswith('pos'):
        positions = list(map(int, anchor_strategy[4:].split('_')))
        anchor_embs = {}
        for _, row in anchors_info.iterrows():
            frame_embs = data_anchor[data_anchor.action_id == row.comments].emb
            anchor_embs[row.end_frame] = [frame_embs.iloc[pos]
                                          for pos in positions]
    else:
        raise ValueError(
            'anchor_strategy "{}" not implemented'.format(anchor_strategy))
...
def calculate_distances(data_anchor, data_target, anchors_info, metric_thr,
                        ...
                        dist_to_anchor_func='mean',    # Mean or median od the distances to anchor
                        last_anchor=True,            # Use last anchor or all of them
                        # Use only best distances [perc0.2]
                        top=False,
                        ):
    # anchors to calculate dists -> { end_frame : list of embs }
    anchor_embs = get_anchor_embs_by_strategy(
        data_anchor, anchors_info, anchor_strategy)

    min_frame = min(anchor_embs.keys())
    detections = {metric: [] for metric, _ in metric_thr.items()}
    for _, row in data_target.iterrows():

        anchors_to_compare = {anchor_frame: embs for anchor_frame,
                              embs in anchor_embs.items() if anchor_frame < row.num_frame}
        if len(anchors_to_compare) == 0:  # Skip comparison if there are no anchors
            for metric, _ in metric_thr.items():
                detections[metric].append(None)
            continue

        if last_anchor:     # Use only the last anchor
            anchors_to_compare = dict(
                [max(anchor_embs.items(), key=lambda x: x[0])])

        # Flatten the anchor embeddings
        anchors_to_compare = sum(anchors_to_compare.values(), [])

        for metric, thrs in metric_thr.items():
            metric_dists = sorted([get_distance(metric, atc, row.emb)
                                  for atc in anchors_to_compare])
...
=======
                    if num_goods == 0:
                        num_goods = 1 if top_perc > 0 else -1
                    metric_dists = metric_dists[:num_goods] if num_goods > 0 else metric_dists[num_goods:]

                else:
                    raise ValueError('top distance param not handled:', top)
...
=======
    ...
    for metric, thr in metric_thr.items():
        data_target.loc[:, metric+'_dist'] = detections[metric]
        data_target.loc[:, metric +
                        '_det'] = data_target[metric+'_dist'] < thr['med']

        data_target.loc[:, metric+'_med'] = data_target[metric +
                                                        '_dist'].apply(lambda x: x < thr['med'] and x > thr['good'])
        data_target.loc[:, metric+'_good'] = data_target[metric +
                                                         '_dist'].apply(lambda x: x <= thr['good'] and x > thr['excel'])
        data_target.loc[:, metric+'_excel'] = data_target[metric +
                                                          '_dist'].apply(lambda x: x <= thr['excel'])

        data_target.loc[:, metric+'_det_level'] = None
        if len(data_target.loc[data_target[metric+'_det'], metric+'_det_level']) > 0:
            data_target.loc[data_target[metric+'_det'], metric+'_det_level'] = data_target[data_target[metric+'_det']][[metric+'_med',
                                                                                                                        metric+'_good', metric+'_excel']].apply(lambda x: np.array(['med', 'good', 'excel'])[x.values][0], axis=1).tolist()
>>>>>>> 4cefc2f (- requirements.txt file with all the dependencies in order to create an python env that can easily run the code)
...
=======
    ...
    # Total frames
    plt.hlines(y=0, xmin=0, xmax=width, color='grey',
               linestyle='-', linewidth=1, zorder=10)
    plt.vlines(0, -VLINE_REF, VLINE_REF, colors='k',
               linestyle='-', linewidth=1, zorder=10)
    plt.vlines(width, -VLINE_REF, VLINE_REF, colors='grey',
               linestyle='-', linewidth=1, zorder=10)
    # Current frames
    plt.hlines(y=0, xmin=0, xmax=curr_frame, color='k',
               linestyle='-', linewidth=1, zorder=10)
    plt.vlines(curr_frame, -VLINE_REF, VLINE_REF, colors='k',
               linestyle='-', linewidth=1, zorder=10)
    # Reference frames
    for i in range(REF_FRAMES_FREQ, total_frames, REF_FRAMES_FREQ):
        plt.vlines(i*width/total_frames, -VLINE_REF, VLINE_REF, colors='grey',
    ...
    # Anchors
    anchors = [(g.tl_frame.min(), g.tl_frame.max())
               for i, g in data_target.groupby('anchor_id')]
    anchors = [(init * width / total_frames, end * width / total_frames)
               for init, end in anchors]
    for init, end in anchors:
        if init > curr_frame:
            continue
        ...
        plt.vlines(init, -VLINE_W, VLINE_W, colors=ANCHOR_COLOR,
                   linestyle='-', alpha=0.6)
        plt.vlines(end, -VLINE_W, VLINE_W, colors=ANCHOR_COLOR,
                   linestyle='-', alpha=0.6)
        plt.hlines(y=0, xmin=init, xmax=end, color=ANCHOR_COLOR,
                   linestyle='-', linewidth=2, alpha=0.6, zorder=15)
    ...
    if len(detections) == 0:
        ...
        detections = list(zip(detections.tl_frame.tolist(),
                          detections[metric+'_det_level']))
        # detections[[metric+'_med', metric+'_good', metric+'_excel']].apply(lambda x: np.array(['med', 'good', 'excel'])[x.values][0], axis=1).tolist()))
    # detections = [ (row.num_frame, color) for _,row in data_target[data_target[metric+'_det'].iterrows() ]
    detections = [(det_frame * width / total_frames, color)
                  for det_frame, color in detections]
    for det_farame, color in detections:
        if det_farame > curr_frame:
            continue
        # plt.vlines(det, -VLINE_W, VLINE_W, colors=DETS_COLOR, linestyle='-', alpha=0.6)
        plt.vlines(det_farame, -VLINE_W, VLINE_W,
                   colors=DETS_COLORS[color], linestyle='-', alpha=0.6)

    # # Targets
    targets = [(g.tl_frame.min(), g.tl_frame.max())
               for i, g in data_target.groupby('target_id')]
    targets = [(init * width / total_frames, end * width / total_frames)
               for init, end in targets]
    for init, end in targets:
        if init > curr_frame:
            continue
        end = min(end, curr_frame)
        plt.vlines(init, -VLINE_W, VLINE_W, colors=TARGET_COLOR,
                   linestyle='-', alpha=0.6)
        plt.vlines(end, -VLINE_W, VLINE_W, colors=TARGET_COLOR,
                   linestyle='-', alpha=0.6)
        plt.hlines(y=0, xmin=init, xmax=end, color=TARGET_COLOR,
                   linestyle='-', linewidth=2, alpha=0.6, zorder=15)
    ...
    plt.text(0, -4.5, data_target.num_frame.min().astype(int), fontsize=7)
    plt.text(100, -4.5, data_target.num_frame.max().astype(int),
             fontsize=7, ha='right')

    canvas = FigureCanvas(fig)
    canvas.draw()       # draw the canvas, cache the renderer
    w, h = canvas.get_width_height()
    frame = np.frombuffer(canvas.tostring_rgb(),
...
=======
    TP_dets = {metric: {t: False for t in data_target.target_id.dropna(
    ).drop_duplicates()} for metric in metric_thr.keys()}
    FP_dets = {metric: [] for metric in metric_thr.keys()}
    for i, row in data_target.iterrows():
        for metric, thr in metric_thr.items():
            ...
            det_frame = row.num_frame
            targets_detected = targets_info[(targets_info.init_frame <= det_frame) & (
                targets_info.end_frame+DIST_TO_GT_TP > det_frame)]
            if len(targets_detected) != 0:
                # TP
                for _, t_row in targets_detected.iterrows():
                    TP_dets[metric][t_row.comments] = True
            else:
                # FP
                if all([det_frame-v > DIST_TO_GT_FP for v in FP_dets[metric]]):
                    FP_dets[metric].append(det_frame)
...
=======
        if TP_pf == FP_pf == 0:
            precision_pf = 1
        elif TP_pf == 0 and FP_pf != 0:
            precision_pf = 0
        elif TP_pf != 0:
            precision_pf = TP_pf / (TP_pf + FP_pf)
        else:
            raise ValueError(
                'Precision exception. TP_pf {}, FP_pf {}'.format(TP_pf, FP_pf))

        if TP_pf == FN_pf == 0:
            recall_pf = 1
        elif TP_pf == 0 and FN_pf != 0:
            recall_pf = 0
        elif TP_pf != 0:
            recall_pf = TP_pf / (TP_pf + FN_pf)
        else:
            raise ValueError(
                'Recall exception. TP_pf {}, FN_pf {}'.format(TP_pf, FN_pf))

        if precision_pf == recall_pf == 0:
            f1_pf = 0
        else:
            f1_pf = (2*precision_pf*recall_pf) / (precision_pf + recall_pf)
...
def draw_skel_bbox(skel, ax, c, label):
    xmin, xmax, zmin, zmax = [
        skel[:, 0].min(), skel[:, 0].max(), skel[:, 2].min(), skel[:, 2].max()]
    y_mean = skel[:, 1].mean()
    ax.plot([xmin, xmax], [y_mean, y_mean], [zmin, zmin], c=c)
    ax.plot([xmin, xmax], [y_mean, y_mean], [zmax, zmax], c=c)
    ax.plot([xmin, xmin], [y_mean, y_mean], [zmin, zmax], c=c)
...
def render_video(data_anchor, data_target, anchors_info, final_stats,
    ...
    data_target['anchor_skel_raw'] = [min(data_anchor.iterrows(), key=lambda r: abs(
        r[1].num_frame - row.num_frame))[1].skels_raw for _, row in data_target.iterrows()]

    total_coords = np.array(
        data_target.anchor_skel_raw.tolist() + data_target.skels_raw.tolist())
    X, Y, Z = total_coords[:, :, 0], total_coords[:,
                                                  :, 2], total_coords[:, :, 1]
    # max_range = np.array([X.max()-X.min(), Y.max()-Y.min(), Z.max()-Z.min()]).max() / 2.0
    max_p, min_p = 99, 1
    max_range = np.array([np.percentile(X, max_p)-np.percentile(X, min_p),
    ...
    sess = anchors_info.iloc[0]['patient'] + '/' + \
        anchors_info.iloc[0]['session'] + ' | ' + \
        anchors_info.iloc[0]['preds_filename']
    max_tl_frame, max_num_frame = data_target.tl_frame.max(), int(
        data_target.num_frame.max())
    num_ex, label_ex = anchors_info.iloc[0].ex_num, anchors_info.iloc[0].action
...
=======
        ...
        # Plot skeleton and floor
        print_3d_skeleton(ax, row.anchor_skel_raw[:, [
                          0, 2, 1]], color=ANCHOR_COLOR)
        print_3d_skeleton(ax, row.skels_raw[:, [0, 2, 1]], color=TARGET_COLOR)
        ax.plot_wireframe(x_floor, y_floor, z_floor, alpha=0.2, rcount=80)

        # Print gt bounding boxes
        if row.is_anchor:
            draw_skel_bbox(row.anchor_skel_raw[:, [
                           0, 2, 1]], ax, c=ANCHOR_COLOR, label='Recording Anchor')
        if row.is_target:
            draw_skel_bbox(
                row.skels_raw[:, [0, 2, 1]], ax, c=TARGET_COLOR, label='Recording Anchor')

        # Print metric distances

        xmin, xmax, zmin, zmax = [row.skels_raw[:, 0].min(), row.skels_raw[:, 0].max(
        ), row.skels_raw[:, 1].min(), row.skels_raw[:, 1].max()]
        y_mean = row.skels_raw[:, 2].mean()
        for i, (metric, thr) in enumerate(metric_thr.items()):
            if not row[metric+'_det']:
                c = 'k'
                ...
                c = DETS_COLORS[row[metric+'_det_level']]
                weight = 'bold'
...
=======
        ...
        frame = Image.fromarray(frame)
        if w_rel < h_rel:
            new_size = (int(w_orig/h_rel), int(h_orig/h_rel))
        else:
            new_size = (int(w_orig/w_rel), int(h_orig/w_rel))
        frame = frame.resize(new_size, Image.ANTIALIAS)
        frame_new = Image.new("RGB", (max_width, max_height), (255, 255, 255))
        frame_new.paste(frame, (max_width-new_size[0], max_height-new_size[1]))
        frame = frame_new
        ...
        for metric in metric_thr.keys():
            ...
            timelines.append(timeline)
        timelines = np.concatenate(timelines, axis=0)
        timelines = timelines[:, ~np.all(
            np.all(timelines == 255, axis=0), axis=1), :]
        # timelines = timelines[~np.all(np.all(timelines==255, axis=1), axis=1), :, :]
        timelines = Image.fromarray(timelines)
        tl_width = int(max_width*0.5)
        tl_height = int(timelines.size[1] * tl_width / timelines.size[0])
        ...
        # num_frame
        # draw.text((20,25), "#{}".format(num_frame+min_frame), fill=(0, 0, 0), font=font)
        draw.text((20, 30), "{:<3} / {} || {:<3} / {}".format(row.tl_frame,
                  max_tl_frame, int(row.num_frame), max_num_frame), fill=(0, 0, 0), font=font)
        # timestamp
        draw.text((20, 60), str(datetime.datetime.fromtimestamp(
            row.ts)), fill=(0, 0, 0), font=font)
        # Exercise
        draw.text((20, 90), "#{} | {}".format(
            num_ex, label_ex), fill=(0, 0, 0), font=font)
...
=======
        ...
        data_target = calculate_distances(
            data_anchor, data_target, anchors_info, metric_thr, **dist_params)

        final_stats = get_evaluation_stats(
            data_target, targets_info, metric_thr, DIST_TO_GT_TP, DIST_TO_GT_FP)
        total_stats.append(final_stats)
        total_labels.append(label)
...
=======
    ...
    first_anchor_embs = get_anchor_embs_by_strategy(
        data_anchor, anchors_info, dist_params['anchor_strategy'])
    first_anchor_embs = {num_frame: emb for num_frame,
                         emb in first_anchor_embs.items() if num_frame <= first_anchor_end}
    first_anchor_embs = sum(list(first_anchor_embs.values()), [])

    # Get init target distances
    init_target_embs = data_target[data_target.num_frame <
                                   first_anchor_end].emb.tolist()
    metric_distances = {metric: [get_distance(metric, a_emb, t_emb)
                                 for t_emb in init_target_embs \
                                 # for num_frame, first_anchor_emb in first_anchor_embs.items()
                                 for a_emb in first_anchor_embs
...
=======
    ...
    if 'perc' in thr_strategy:
        perc = int([s for s in thr_strategy.split('_') if 'perc' in s][0][4:])
        if 'fac' in thr_strategy:
            fac = float(
                [s for s in thr_strategy.split('_') if 'fac' in s][0][4:])
        else:
            fac = 1
        thr = {metric: np.percentile(
            metric_distances[metric], perc)*fac for metric in metrics}

        if 'max' in thr_strategy:
            max_value = float(
                [s for s in thr_strategy.split('_') if 'max' in s][0][3:])
            thr = {metric: min(thr[metric], max_value) for metric in metrics}

        metric_thr = {metric: {'med': thr[metric], 'good': 0, 'excel': 0}
                      for metric in metrics}
    else:
        raise ValueError('thr_strategy not handled:', thr_strategy)
...
=======
    ...
    results_folder = os.path.join(
        '/mnt/hdd/ml_results/core_renders/', *path_model.split('/')[-3:])
    if not os.path.isdir(results_folder):
        os.makedirs(results_folder)

    timelines_folder = results_folder + \
        'timelines_{}-{}/'.format(batch, thr_strategy)
    if not os.path.isdir(timelines_folder):
        os.makedirs(timelines_folder)
...
=======
            ...
            if store_timelines or plot_timelines:
                timeline = get_current_timeline(None, data_target, metric,
                                                ...
                                                width=100, plot=False)
                timelines.append(timeline)
            print('{:<2} / {} || {:<4} || {}'.format(num_file+1,
                  len(pred_files), metric, final_stats[metric]['stats_str']))
